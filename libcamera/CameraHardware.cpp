...
void CameraHardware::initHeapLocked()
{
	...
	//int how_big = preview_width * preview_height * 2; // YUV422
	int how_big = (preview_width * preview_height * 3) >> 1; // YUV420
	...
	//mEncodeFrameSize = preview_width * preview_height * 2;	//YUV422
	mEncodeFrameSize = (preview_width * preview_height * 3) >>1;	//YUV420
...
int CameraHardware::pictureThread()
{
	...
	if (mMsgEnabled & CAMERA_MSG_RAW_IMAGE) {
		...
		CAMHW_DEBUG_LOGD("mCameraPixelFormat = 0x%x",
			mCameraPixelFormat);
		...
		mDataCb(CAMERA_MSG_RAW_IMAGE, mCamera.GrabRawFrame(mCameraPixelFormat), mCallbackCookie);
	}

	if (mMsgEnabled & CAMERA_MSG_COMPRESSED_IMAGE) {
		LOGI("---CAMERA_MSG_COMPRESSED_IMAGE---\n");
		mDataCb(CAMERA_MSG_COMPRESSED_IMAGE, mCamera.GrabJpegFrame(mCameraPixelFormat),
				mCallbackCookie);
	}
...
status_t CameraHardware::dump(int fd, const Vector<String16>& args) const
{
	...
	if (mFakeCamera != 0) {
		...
		snprintf(buffer, 255, " preview frame(%d), size (%d), running(%s)\n",
			mCurrentPreviewFrame, mPreviewFrameSize, !mPreviewStopped?"true": "false");
...
status_t CameraHardware::setParameters(const CameraParameters& params)
{
	...
	PRINT_DEBUG_MSG;
		mCameraPixelFormat = V4L2_PIX_FMT_NV16;
	} else if (strcmp(params.getPreviewFormat(), "yuv420sp") == 0) {
	PRINT_DEBUG_MSG;
		mCameraPixelFormat = V4L2_PIX_FMT_NV12;
	} else if (strcmp(params.getPreviewFormat(), "yuv422p") == 0) {
	PRINT_DEBUG_MSG;
		mCameraPixelFormat = V4L2_PIX_FMT_YUV422P;
	} else if (strcmp(params.getPreviewFormat(), "yuv420p") == 0) {
	PRINT_DEBUG_MSG;
		mCameraPixelFormat = V4L2_PIX_FMT_YUV420;
	}else{
...
#if DEBUG_CAM
LOGD("CameraHardware::%s:%d: mCameraPixelFormat=0x%x,mCameraPixelFormat=%d",
		__FUNCTION__,__LINE__,mCameraPixelFormat,mCameraPixelFormat);
#endif
